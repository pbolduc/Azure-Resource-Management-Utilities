...
namespace Microsoft.Azure.Management.Resource.Fluent.Authentication
{
    ...
    public class AzureCredentials : ServiceClientCredentials
    {
        ...
        private readonly CertificateLoginInformation certificateLoginInformation;
            ...
            : this(tenantId, environment)
        {
         ...
         public AzureCredentials(CertificateLoginInformation certificateLoginInformation, string tenantId, AzureEnvironment environment)
            : this(tenantId, environment)
        {
            this.certificateLoginInformation = certificateLoginInformation;
        }
...
#endif
                else if (certificateLoginInformation != null)
                {
                    var clientAssertion = new ClientAssertionCertificate(certificateLoginInformation.ClientId, certificateLoginInformation.Certificate);
                    credentialsCache[adSettings.TokenAudience] = await ApplicationTokenProvider.LoginSilentAsync(TenantId, clientAssertion, adSettings, TokenCache.DefaultShared);
                }
            }
