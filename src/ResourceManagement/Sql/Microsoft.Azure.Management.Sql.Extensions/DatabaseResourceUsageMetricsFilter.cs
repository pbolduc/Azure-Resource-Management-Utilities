namespace Microsoft.Azure.Management.Sql
{
    ...
    public class DatabaseResourceUsageMetricsFilter
    {
        ...
        public DatabaseResourceUsageMetricsFilter(DateTime startTime, DateTime endTime, TimeSpan grain, params string[] metrics)
        {
            StartTime = startTime;
            EndTime = endTime;
            TimeGrain = grain;
        ...
        private void AppendTimeGrain(StringBuilder filterBuilder)
        {
            if (TimeGrain.HasValue)
            {
                ...
                if (TimeSpan.FromHours(1) < EndTime - StartTime)
                {
                    // must be multiple of 5 minutes, round up to 
                    grain = grain.RoundToMinutes(5);
                }
                else
                {
                    grain = grain.RoundToSeconds(15);
                }
    ...
    {
        public static TimeSpan RoundToMinutes(this TimeSpan value, int n)
        {
            return TimeSpan.FromMinutes(n * Math.Ceiling(value.TotalMinutes / n));
        }

        public static TimeSpan RoundToSeconds(this TimeSpan value, int n)
        {
